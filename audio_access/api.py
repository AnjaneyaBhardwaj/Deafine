...
async def process_audio_file(
    file_path: Path, config: Config, chunk_duration: int = 5
...
async def transcribe_audio(
    ...
    Args:
        ...
        num_speakers: Max speakers to detect (default: 2)
    ...
    try:
        ...
        # Process audio
        segments = await process_audio_file(file_path, config, chunk_duration)
...
async def transcribe_stream(
    ...
    background_tasks.add_task(
        process_session_async, session_id, file_path, eleven_api_key
    )
...
async def process_session_async(
    session_id: str, file_path: Path, api_key: Optional[str] = None
):
...
async def get_session_transcript(session_id: str):
    ...
    if session["status"] == "processing":
        raise HTTPException(
            status_code=425, detail="Session still processing. Check back later."
        )
...
async def websocket_transcribe(websocket: WebSocket):
    ...
    3. Client sends: {"command": "set_name", "user_name": "John"} (optional, for haptics)
    4. Client sends: {"command": "set_num_speakers", "num_speakers": 3} (optional, default: 2)
    5. Client sends: binary audio chunks (PCM 16-bit, 16kHz, mono)
    6. Server sends: {"type": "transcript", "segment": {...}}
    7. Server sends: {"type": "haptic", "reason": "name_mentioned"} (if name detected)
    8. Server sends: {"type": "status", "message": "..."}
    9. On disconnect: {"type": "summary", "data": {...}}
    """
    ...
    try:
        ...
        if not config.eleven_api_key:
            await websocket.send_json(
                {"type": "error", "message": "ELEVEN_API_KEY not configured on server"}
            )
        ...
        try:
            while True:
                ...
                if "bytes" in data:
                    ...
                    websocket_sessions[session_id]["timestamp"] += (
                    ...
                    if await transcriber.should_process(frame.timestamp):
                        ...
                        for segment in segments:
                            ...
                            if name_mentioned:
                                ...
                                print(
                                    f"📳 [{session_id}] HAPTIC: {user_name} mentioned by {segment.speaker_id}"
                                )

                            print(
                                f"📝 [{session_id}] {segment.speaker_id}: {segment.text}"
                            )
                    ...
                    if message.get("command") == "ping":
                        await websocket.send_json(
                            {
                                "type": "pong",
                                "timestamp": websocket_sessions[session_id][
                                    "timestamp"
                                ],
                            }
                        )
                    ...
                    elif message.get("command") == "set_num_speakers":
                        # Set number of speakers for diarization
                        num_speakers = int(message.get("num_speakers", 2))
                        config.num_speakers = num_speakers
                        await websocket.send_json(
                            {
                                "type": "config_confirmed",
                                "num_speakers": num_speakers,
                                "message": f"Number of speakers set to: {num_speakers}",
                            }
                        )
                        print(
                            f"🎙️ [{session_id}] Number of speakers set: {num_speakers}"
                        )
