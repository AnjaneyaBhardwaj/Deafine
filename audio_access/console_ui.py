...
class ConsoleUI:
    ...
    def generate_display(self) -> Panel:
        ...
        for speaker_id, display in sorted_speakers:
            ...
            caption = (
                display.current_caption[-300:]
                if display.current_caption
        ...
        if self.overlap_active:
            overlap_text = Text(
                f"[OVERLAP] {', '.join(self.overlap_speakers)}", style="bold red blink"
            )
        ...
        title = (
            f"🎤 Audio Access - Live Transcription (ElevenLabs) [{mins:02d}:{secs:02d}]"
        )

        panel = Panel(table, title=title, border_style="bright_blue", padding=(1, 2))
